  ...
  - Intros vals' vers'; rewrite <- size_def in *.
    ...
    { split; intros.
      ...
      + simpl; rewrite version_T_eq.
        ...
        * rewrite !sepcon_assoc, (sepcon_comm P), <- !sepcon_assoc, (sepcon_assoc _ _ P); etransitivity;
            [|apply view_shift_sepcon2, HP].
...
Proof.
  ...
  { simpl; split; intros; rewrite !emp_sepcon.
    ...
    rewrite version_T_eq; view_shift_intro L0; view_shift_intros.
    rewrite <- !sepcon_assoc, (sepcon_comm _ (ghost _ _)), <- !sepcon_assoc, (sepcon_comm _ (ghost_snap _ _)).
    rewrite <- !sepcon_assoc, snap_master_join by auto.
    rewrite sepcon_assoc; etransitivity; [apply view_shift_sepcon2|].
    { if_tac; [rewrite sepcon_emp; reflexivity|].
      rewrite <- sepcon_map; apply view_shift_sepcon_list; [rewrite 2Zlength_map; auto|].
      rewrite Zlength_map; intros.
      erewrite !Znth_map, !Znth_upto by (auto; rewrite Zlength_upto in *; omega).
      etransitivity; [apply protocol_R_absorb; auto|].
      view_shift_intros; reflexivity. }
    unfold protocol_W; apply derives_view_shift; entailer!. }
  ...
  forward_call_dep [Z : Type] (store_rel_witness version (v0 + 1) v0 (v0 + 1) Z.le
    ...
    (fold_right sepcon emp (map (fun i => protocol_W (Znth i locs Vundef) (map_Znth i L 0) map_incl
      (data_T version locs g, data_T version locs g)) (upto (Z.to_nat size))) * ghost (gsh2, L) g)).
  { simpl; cancel. }
  { split; auto; split; [|split; [|omega]]; intros; simpl; rewrite ?emp_sepcon; [reflexivity|].
